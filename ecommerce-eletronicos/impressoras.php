...
<title>Configuracao de Impressoras</title>
<?= $tema ?>
...
.imp-card-icon { width:42px; height:42px; border-radius:10px; background:rgba(255,255,255,.2); display:flex; align-items:center; justify-content:center; font-size:18px; flex-shrink:0; color:#fff; font-weight:700; }
...
<div class="imp-page">
  ...
  <div class="imp-topbar">
    <a href="admin.php" class="imp-btn-back">&larr; Voltar ao Admin</a>
  ...
  <div class="imp-hero">
    <div>
      <h1>Configuracao de Impressoras</h1>
...
<div class="imp-save-bar" id="saveBar">
  <div class="imp-save-info">Voce tem alteracoes <strong>nao salvas</strong></div>
...
<div class="imp-overlay" id="overlayDel">
  <div class="imp-modal">
    ...
    <div class="imp-modal-sub" id="modalDelSub">As categorias atribuidas serao liberadas.</div>
...
const CATS = CATS_RAW.map((c, i) => ({
  ...
  nome: c.nome,
  cor:  CORES_FALLBACK[i % CORES_FALLBACK.length],
  bg:   BG_FALLBACK[i % BG_FALLBACK.length],
}));
...
async function init() {
  ...
  try {
    ...
    try { data = JSON.parse(txt); }
      ...
      console.error('Carregar - resposta nao e JSON:', txt.substring(0, 300));
      toast('Arquivo deve ser .php - PHP nao esta sendo executado', 'error');
      return;
    }
    if (data.success) {
      imps   = data.data;
    ...
    console.error('Erro de conexao ao carregar:', e);
    toast('Erro de conexao ao carregar impressoras', 'error');
  }
...
function selCat(catId) {
  ...
  else        toast('Selecao cancelada', 'info');
}
...
function atribuir(impId, catId) {
  ...
  if (imp && !imp.cats.includes(cid)) {
    ...
    toast(`"${cat?.nome}" atribuida a "${imp.nome || 'Impressora'}"`, 'success');
  }
  catSel = null;
...
function upd(id, campo, val) {
  ...
  if (campo === 'ip' || campo === 'porta') {
    ...
    if (el && imp2) el.textContent = imp2.ip ? `${imp2.ip}:${imp2.porta}` : 'IP nao configurado';
  }
}
...
function renderImps() {
  ...
  if (!imps.length) {
    grid.innerHTML = `<div class="imp-empty">
      <div class="imp-empty-icon" style="font-size:40px;opacity:.3;">&#128424;</div>
      <div class="imp-empty-txt">Nenhuma impressora configurada</div>
      <div class="imp-empty-sub">Clique em <strong>Nova Impressora</strong> para comecar</div>
    </div>`;
  ...
  imps.forEach(imp => {
    ...
    const ipExib   = imp.ip ? `${esc(imp.ip)}:${esc(imp.porta)}` : 'IP nao configurado';
    ...
    const catsHtml = imp.cats.length
      ? imp.cats.map(cid => {
          ...
          return `<span class="imp-cat-tag" style="background:${c.bg};color:${c.cor};">
            ${esc(c.nome)}
            <span class="rm" onclick="event.stopPropagation();removerCat(${imp.id},'${cid}')" title="Remover">&times;</span>
          </span>`;
        }).join('')
    ...
    card.innerHTML = `
      ...
      <div class="imp-card-body">
        ...
        <div class="imp-drop-label">Categorias desta impressora</div>
      ...
      <div class="imp-card-foot">
        ...
        <button class="imp-btn-del" onclick="abrirDel(${imp.id})" title="Remover">
...
async function testar(id) {
  ...
  try {
    ...
    toast(d.success ? `"${imp.nome}" respondeu com sucesso!` : `Falha: ${d.message}`, d.success ? 'success' : 'error');
  } catch(e) {
    imp.status = 'offline';
...
function abrirDel(id) {
  delId = id;
  const imp = imps.find(p => p.id === id);
  document.getElementById('modalDelSub').textContent = `Remover "${imp?.nome || 'impressora'}"? As categorias serao liberadas.`;
...
async function salvarTudo() {
  ...
  try {
    ...
    try { data = JSON.parse(txt); }
      ...
      console.error('Resposta do servidor (nao e JSON):', txt.substring(0, 300));
      toast('Erro: servidor retornou HTML. Verifique se o arquivo e .php', 'error');
      btn.innerHTML = '<svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Salvar Configuracoes';
      btn.disabled = false;
      return;
    }
    if (data.success) {
    ...
    toast('Erro de conexao ao salvar: ' + e.message, 'error');
  }
  btn.innerHTML = '<svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Salvar Configuracoes';
  btn.disabled = false;
}

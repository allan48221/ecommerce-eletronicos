...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'criar') {
    ...
    if ($nome === '') {
        ...
        $conn->prepare("INSERT INTO adicionais (nome, preco) VALUES (?, ?)")->execute([$nome, $preco]);
...
$adicionais = $conn->query("SELECT * FROM adicionais ORDER BY criado_em DESC")->fetchAll();
$produtos   = $conn->query("SELECT id_produto, nome FROM produtos WHERE ativo=TRUE ORDER BY nome")->fetchAll();
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hero {
            ...
            border-radius: 18px; padding: 28px 30px 24px; color: #fff;
        ...
        .campo input {
            ...
            font-family: 'Sora', sans-serif;
            color: #0f172a; outline: none; transition: .2s; background: #fff;
        }
        .campo input:focus { border-color: var(--primary, #2563eb); box-shadow: 0 0 0 3px rgba(37,99,235,.1); }

        .btn-primary {
            padding: 12px 22px; background: var(--primary, #2563eb); color: #fff;
            border: none; border-radius: 9px; font-size: 14px; font-weight: 700;
            font-family: 'Sora', sans-serif; cursor: pointer; transition: .2s; width: 100%;
        }
        ...
        @media (max-width: 600px) {
            ...
            .campo input { padding: 13px; font-size: 16px; }
            .btn-primary { padding: 14px; font-size: 15px; }

            /* Linha do adicional: info em cima, botoes embaixo */
            .adicional-row {
            ...
            /* Checkboxes ocupam mais largura */
            .check-label { width: 100%; font-size: 14px; padding: 11px 14px; }
            .checkboxes-grid { flex-direction: column; gap: 6px; }

            /* Salvar vinculo full width */
            .btn-salvar-vinculo { width: 100%; padding: 13px; font-size: 14px; }
            ...
            .btn-cinza,
            .modal-footer .btn-primary { width: 100%; padding: 14px; }

            .produto-vinculo-card { padding: 14px; }
            .produto-vinculo-nome { font-size: 13px; }
        }
...
<div class="page">
                ...
                <div class="adicional-row <?= !$a['ativo'] ? 'inativo' : '' ?>">
                    <div style="flex:1;min-width:0;width:100%;">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;margin-top:4px;flex-wrap:wrap;">
                            <span class="adicional-preco">
                                <?= $a['preco'] > 0 ? '+ R$ ' . number_format($a['preco'],2,',','.') : 'Gratis' ?>
                            </span>
                            <span class="<?= $a['ativo'] ? 'badge-ativo' : 'badge-inativo' ?>">
                    ...
                    <div class="adicional-acoes">
                        ...
                        <form method="POST" action="adicionais_admin.php" style="display:contents;">
                            ...
                            <button type="submit" class="btn-sm <?= $a['ativo'] ? 'btn-toggle-off' : 'btn-toggle-on' ?>">
                                <?= $a['ativo'] ? 'Desativar' : 'Ativar' ?>
                            </button>
                        </form>
                        <form method="POST" action="adicionais_admin.php" style="display:contents;"
                              onsubmit="return confirm('Excluir este adicional? Os vinculos com produtos tambem serao removidos.')">
    ...
    <!-- VINCULAR ADICIONAIS AOS PRODUTOS -->
    <?php if (!empty($adicionais) && !empty($produtos)): ?>
    <div class="card">
        <div class="card-title">Vincular adicionais a produtos</div>
        <p style="font-size:13px;color:#64748b;margin-bottom:20px;">
            Marque quais adicionais cada produto aceita e clique em Salvar. So adicionais ativos aparecem.
        </p>

        <?php if (empty($ativos)): ?>
            <div class="vazio">Todos os adicionais estao desativados. Ative pelo menos um para vincular.</div>
        <?php else: ?>
    ...
    <div class="card" style="text-align:center;padding:40px;">
        <div style="font-size:15px;color:#94a3b8;font-weight:600;margin-bottom:8px;">
            Nenhum adicional criado ainda.
        </div>
        <div style="font-size:13px;color:#cbd5e1;">
            Crie pelo menos um adicional acima para comecar a vincular aos produtos.
        </div>
    </div>
    <?php endif; ?>
...
function toggleLabel(cb) {
    ...
    if (label) label.classList.toggle('marcado', cb.checked);
}
</script>
</body>
</html>
 ...
 if (empty($nome)) {
    ...
    if (empty($mensagem)) {
       $imagem_principal = !empty($imagens_salvas) ? $imagens_salvas[0] : null;
        ...
        $sql  = "INSERT INTO produtos (nome, descricao, marca, modelo, preco, preco_promocional, estoque, id_categoria, imagem, destaque, codigo_barras)
        ...
        if (!$stmt) {
            ...
            $ok = $stmt->execute([$nome, $descricao, $marca, $modelo, $preco, $preco_promocional, $estoque, $id_categoria, $imagem_principal, $destaque, $codigo_barras ?: null]);
...
<head>
    ...
    <style>
        ...
        /* ── Upload area ── */
        .upload-area {
            border: 2px dashed #c4b5fd; border-radius: 0.75rem;
            padding: 1.5rem; background: #faf5ff;
        ...
        .preview-item img {
            ...
            border-radius: 8px; border: 2px solid #ddd6fe; display: block;
        }
        .preview-item .btn-remover {
        ...
        .field-check {
            ...
            padding: 0.85rem 1rem; background: var(--light);
            border-radius: 0.65rem; border: 1.5px solid #ddd;
        ...
        .btn-cadastrar:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 6px 20px rgba(124,58,237,0.45); }
        .btn-back {
        ...
        .crop-modal-box {
            background: #fff; border-radius: 1rem; padding: 1rem; width: min(420px, 92vw);
        ...
        #crop-preview-canvas { width: 60px; height: 60px; border-radius: 6px; border: 2px solid #ddd6fe; display: block; image-rendering: auto; }
        .crop-actions { display: flex; gap: 0.75rem; justify-content: flex-end; }
        .btn-crop-cancel { padding: 0.5rem 1rem; border-radius: 0.5rem; border: 1.5px solid #ddd; background: #fff; color: var(--gray); font-weight: 600; cursor: pointer; font-size: 0.85rem; transition: background 0.15s; }
        .btn-crop-cancel:hover { background: #f1f5f9; }
        .btn-crop-ok { padding: 0.5rem 1.2rem; border-radius: 0.5rem; border: none; background: linear-gradient(135deg, var(--primary), var(--purple)); color: white; font-weight: 700; cursor: pointer; font-size: 0.85rem; transition: opacity 0.15s; }
        .btn-crop-ok:hover { opacity: 0.88; }
        ...
        @media (max-width: 480px) {
            ...
            .form-produto { gap: 0.85rem; margin-top: 1rem; }
            .form-produto .field input,
            .form-produto .field select,
            .form-produto .field textarea { padding: 0.6rem 0.75rem; font-size: 0.85rem; }
            .form-produto .field textarea { min-height: 80px; }
            .upload-area { padding: 1rem 0.75rem; }
            .upload-area p { font-size: 0.75rem; }
            .btn-upload { font-size: 0.8rem; padding: 0.5rem 1rem; }
            .btn-cadastrar { padding: 0.85rem; font-size: 0.95rem; }
            .btn-back { font-size: 12px; padding: 7px 13px; }
            .preview-item img { width: 72px; height: 72px; }
            .field-check { padding: 0.7rem 0.75rem; }
            .field-check span { font-size: 0.85rem; }
            .crop-modal-box { padding: 0.75rem; gap: 0.6rem; width: 96vw; }
            .crop-modal-title { font-size: 0.88rem; }
            .crop-modal-sub { font-size: 0.72rem; }
            .btn-crop-cancel, .btn-crop-ok { font-size: 0.8rem; padding: 0.45rem 0.9rem; }
        }
    </style>
</head>
<body>
<div class="container">
    ...
    <div class="card">
        ...
        <form id="form-produto" method="POST" enctype="multipart/form-data" class="form-produto">
            ...
            <div class="field">
                <label>Descricao <span></span></label>
            ...
            <div class="form-row-3">
                <div class="field">
                    ...
                    <div class="preco-wrap">
                        ...
                        <input type="text" inputmode="numeric" name="preco"
                               id="inp-preco" data-preco value="0,00" required autocomplete="off">
                    </div>
                </div>
                <div class="field">
                    ...
                    <div class="preco-wrap">
                        <span class="preco-rs">R$</span>
                        <input type="text" inputmode="numeric" name="preco_promocional"
                               id="inp-preco-promo" data-preco value="0,00" autocomplete="off">
                    </div>
                </div>
                <div class="field">
...
/* ═══════════════════════════════════════
   ...
   Digita somente numeros, virgula fixa
═══════════════════════════════════════ */
...
document.getElementById('input-imagens').addEventListener('change', function(){
    filesFila = Array.from(this.files);
    this.value = '';
    if (!filesFila.length) return;
    cropIndex = 0;
    proximoCrop();
});

function proximoCrop() {
    if (cropIndex >= filesFila.length) { atualizarPreview(); return; }
    const r = new FileReader();
    r.onload = e => abrirCrop(e.target.result);
    r.readAsDataURL(filesFila[cropIndex]);
}
...
function initCanvas() {
    ...
    CW = cropImg.naturalWidth;
    CH = cropImg.naturalHeight;
    canvas.width  = CW;
    canvas.height = CH;
...
function renderTudo(ctx) {
    if (!ctx) ctx = document.getElementById('crop-canvas').getContext('2d');
    const canvas = document.getElementById('crop-canvas');
    const vf = canvas.width / canvas.getBoundingClientRect().width;
    ctx.drawImage(cropImg, 0, 0, CW, CH);
    ctx.fillStyle = 'rgba(0,0,0,0.50)';
    ctx.fillRect(0, 0, CW, box.y);
    ctx.fillRect(0, box.y+box.s, CW, CH-(box.y+box.s));
    ctx.fillRect(0, box.y, box.x, box.s);
    ctx.fillRect(box.x+box.s, box.y, CW-(box.x+box.s), box.s);
    ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 2 * vf;
    ctx.strokeRect(box.x, box.y, box.s, box.s);
    ctx.strokeStyle = 'rgba(255,255,255,0.35)'; ctx.lineWidth = 1 * vf;
    for (let i = 1; i < 3; i++) {
    ...
    const handles = getHandleRects(vf);
    ctx.fillStyle = '#ffffff'; ctx.strokeStyle = 'rgba(124,58,237,0.9)'; ctx.lineWidth = 2 * vf;
    handles.forEach(h => { ctx.fillRect(h.x, h.y, h.w, h.h); ctx.strokeRect(h.x, h.y, h.w, h.h); });
}
...
function getHandleRects(vf) {
    if (!vf) vf = getVF();
    const hs = 10 * vf, { x, y, s } = box, mx = x + s/2 - hs/2, my = y + s/2 - hs/2;
    return [
        {id:'tl',x:x-hs/2,y:y-hs/2,w:hs,h:hs},{id:'tr',x:x+s-hs/2,y:y-hs/2,w:hs,h:hs},
...
function confirmarCrop() {
    ...
    out.getContext('2d').drawImage(cropImg, box.x/scale, box.y/scale, box.s/scale, box.s/scale, 0, 0, OUT, OUT);
    out.toBlob(function(blob) {
        const nome = filesFila[cropIndex]?.name || ('img_'+Date.now()+'.jpg');
        const ext  = nome.split('.').pop().toLowerCase();
        const mime = ext==='png'?'image/png':ext==='gif'?'image/gif':'image/jpeg';
        arquivosSelecionados.push(new File([blob], nome, { type: mime }));
        document.getElementById('crop-modal').classList.remove('open');
        cropIndex++;
        setTimeout(proximoCrop, 80);
    }, 'image/jpeg', 0.92);
}

function atualizarPreview() {
    const wrap = document.getElementById('preview-imagens'), cnt = document.getElementById('contador-imagens');
    wrap.innerHTML = '';
    arquivosSelecionados.forEach(function(file, i) {
        const url = URL.createObjectURL(file);
        const item = document.createElement('div'); item.className='preview-item';
        const img  = document.createElement('img'); img.src=url; img.title=file.name;
        img.onload = () => URL.revokeObjectURL(url);
        const btn  = document.createElement('button'); btn.className='btn-remover'; btn.type='button'; btn.innerHTML='&times;';
        btn.onclick = () => { arquivosSelecionados.splice(i,1); atualizarPreview(); };
        item.appendChild(img); item.appendChild(btn); wrap.appendChild(item);
    });
    cnt.textContent = arquivosSelecionados.length ? arquivosSelecionados.length + ' imagem(ns) selecionada(s).' : '';
}
...
function enviarFormulario() {
    ...
    fd.delete('imagens[]');
    arquivosSelecionados.forEach(f => fd.append('imagens[]', f));
    ...
    fetch(window.location.href,{method:'POST',body:fd})
        ...
        .catch(err=>{alert('Erro: '+err);btn.textContent='Cadastrar Produto';btn.disabled=false;});
}

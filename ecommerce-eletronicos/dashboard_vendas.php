...
// CARDS DE RESUMO (exclui cancelados)
$stmt = $conn->prepare("
    SELECT 
...
// COMPARATIVO: periodo anterior (exclui cancelados)
...
// FATURAMENTO DO DIA (hoje, independente do filtro)
...
// PRODUTOS MAIS VENDIDOS (exclui cancelados)
...
// CATEGORIAS MAIS VENDIDAS (exclui cancelados)
...
// ULTIMOS PEDIDOS (todos os status para visibilidade, com tipo)
...
// GRAFICO: vendas por dia (exclui cancelados)
...
// FORMAS DE PAGAMENTO (exclui cancelados)
...
// CONTAGEM: pedidos cancelados no periodo
$stmt_canc = $conn->prepare("SELECT COUNT(*) FROM pedidos WHERE data_pedido BETWEEN ? AND ? AND status='cancelado'");
$stmt_canc->execute([$di, $df]);
$total_cancelados = $stmt_canc->fetchColumn();

// VENDAS POR ATENDENTE/CAIXA (lançado_por em comanda_itens)
...
// PRODUTOS POR ATENDENTE (para o modal)
...
<head>
    ...
    <style>
        ...
        .dash-title { font-size: 1.15rem; font-weight: 800; color: var(--dark); display: flex; align-items: center; gap: 0.5rem; }
        @media (min-width: 769px) { .dash-title { font-size: 1.75rem; } }
        ...
        @media (min-width: 769px) { .card-resumo { padding: 1.5rem; border-radius: 1rem; } .card-resumo:hover { transform: translateY(-3px); transition: transform 0.2s; } }
        ...
        @media (min-width: 769px) { .card-resumo::before { width: 80px; height: 80px; } }
        .card-resumo.azul::before    { background: var(--primary); }
        .card-resumo.verde::before   { background: var(--success); }
        .card-resumo.roxo::before    { background: var(--purple); }
        .card-resumo.vermelho::before { background: var(--danger); }
        ...
        @media (min-width: 769px) { .card-resumo-var { font-size: 0.8rem; } }
        .card-resumo-var.up     { color: var(--success); }
        .card-resumo-var.down   { color: var(--danger); }
        .card-resumo-var.neutro { color: var(--gray); }
        ...
        @media (min-width: 769px) {
            .dash-grid-2 { grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem; }
            .dash-grid-3 { grid-template-columns: 2fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem; }
        }
        ...
        @media (min-width: 769px) { .dash-card-title { font-size: 1rem; margin-bottom: 1.25rem; } }

        .ranking-table { width: 100%; border-collapse: collapse; }
        .ranking-table th { text-align: left; font-size: 0.68rem; font-weight: 700; color: var(--gray); text-transform: uppercase; letter-spacing: 0.4px; padding: 0.4rem 0.5rem; background: transparent; }
        ...
        @media (min-width: 769px) { .ranking-table th { font-size: 0.75rem; padding: 0.6rem 0.75rem; } .ranking-table td { font-size: 0.88rem; padding: 0.75rem; } }
        ...
        @media (min-width: 769px) { .prod-img-mini { width: 36px; height: 36px; } }
        ...
        @media (min-width: 769px) { .badge-status { font-size: 0.75rem; padding: 0.2rem 0.6rem; } }
        ...
        .cat-bar-nome  { font-size: 0.78rem; font-weight: 600; color: var(--dark); }
        .cat-bar-valor { font-size: 0.72rem; color: var(--primary); font-weight: 700; }
        ...
        @media (min-width: 769px) { .cat-bar-nome { font-size: 0.85rem; } .cat-bar-valor { font-size: 0.82rem; } .cat-bar-bg { height: 8px; } .cat-bar-sub { font-size: 0.75rem; } }
        ...
        .pag-icone { font-size: 1.1rem; margin-right: 0.45rem; }
        .pag-nome  { font-size: 0.8rem; font-weight: 600; color: var(--dark); }
        .pag-qtd   { font-size: 0.7rem; color: var(--gray); }
        .pag-valor { font-size: 0.82rem; font-weight: 700; color: var(--success); }
        @media (min-width: 769px) { .pag-icone { font-size: 1.4rem; } .pag-nome { font-size: 0.9rem; } .pag-qtd { font-size: 0.78rem; } .pag-valor { font-size: 0.95rem; } }
        ...
        @media (min-width: 769px) {
            .pedido-row { display: flex; align-items: center; justify-content: space-between; gap: 0.5rem; flex-wrap: wrap; padding: 0.75rem 0; }
            .pedido-id   { font-size: 0.88rem; }
            .pedido-nome { font-size: 0.88rem; flex: 1; white-space: normal; }
            .pedido-val  { font-size: 0.9rem; text-align: left; }
            .pedido-sub  { display: none; }
            .pedido-badge { text-align: left; }
            .pedido-pag  { display: inline; font-size: 0.82rem; color: var(--gray); }
            .pedido-data-full { display: inline; font-size: 0.78rem; color: var(--gray); }
        }
        ...
        /* Aviso cancelados */
        .aviso-cancelados { background: #fef2f2; border: 1px solid #fecaca; border-radius: 0.6rem; padding: 0.6rem 1rem; font-size: 0.78rem; color: #991b1b; font-weight: 600; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; }
...
<body>
    ...
    <div class="dash-wrapper">
        ...
        <div class="filtro-card">
            ...
            <div class="periodo-shortcuts">
                ...
                $mes_ini = date('Y-m-01');
                $mes_3   = date('Y-m-d', strtotime('-89 days'));
        ...
        <!-- CARDS DE RESUMO -->
        <?php
        $var_vendas  = variacao($resumo['total_vendido'], $anterior['total_ant']);
        $var_pedidos = variacao($resumo['total_pedidos'], $anterior['pedidos_ant']);
        ?>
        <div class="cards-resumo">
            <div class="card-resumo azul">
                ...
                <div class="card-resumo-var <?= $var_vendas >= 0 ? 'up' : 'down' ?>">
                    <?= $var_vendas >= 0 ? '↑' : '↓' ?> <?= abs($var_vendas) ?>%
                </div>
            </div>
            <div class="card-resumo verde">
                ...
                <div class="card-resumo-var <?= $var_pedidos >= 0 ? 'up' : 'down' ?>">
                    <?= $var_pedidos >= 0 ? '↑' : '↓' ?> <?= abs($var_pedidos) ?>%
                </div>
            </div>
            <div class="card-resumo roxo">
                ...
                <div class="card-resumo-var neutro">
                    <?= $faturamento_hoje['pedidos_hoje'] ?> pedido(s) hoje
                </div>
            </div>
        </div>
        ...
        <div class="dash-grid-3">
            <div class="dash-card">
                ...
                <?php else: ?>
                    <div class="empty-dash">Nenhuma venda no periodo.</div>
                <?php endif; ?>
            </div>
            <div class="dash-card">
                ...
                <?php if (count($pagamentos) > 0):
                    ...
                    foreach ($pagamentos as $pag):
                        $icone = $icones_pag[$pag['forma_pagamento']] ?? '';
                ?>
                    <div class="pag-item">
                        <div style="display:flex;align-items:center;">
                            <span class="pag-icone"><?= $icone ?></span>
                            <div>
                                <div class="pag-nome"><?= htmlspecialchars($pag['forma_pagamento']) ?></div>
                                <div class="pag-qtd"><?= $pag['qtd'] ?> pedido(s)</div>
                            </div>
                        </div>
                        <div class="pag-valor">R$ <?= number_format($pag['total'], 2, ',', '.') ?></div>
                    </div>
                <?php endforeach; else: ?>
                    <div class="empty-dash">Sem dados.</div>
                <?php endif; ?>
            </div>
        </div>
                        ...
                        <tr>
                            <td>
                                <div class="prod-info-mini">
                                    <img src="uploads/<?= htmlspecialchars($prod['imagem'] ?: 'placeholder.jpg') ?>"
                                         class="prod-img-mini"
                                         onerror="this.src='uploads/placeholder.jpg'">
                                    <span style="font-weight:600;"><?= htmlspecialchars($prod['nome']) ?></span>
                                </div>
                            </td>
                ...
                ?>
                    <div class="cat-bar-item">
                        <div class="cat-bar-header">
                            <span class="cat-bar-nome"><?= htmlspecialchars($cat['categoria']) ?></span>
                            <span class="cat-bar-valor">R$ <?= number_format($cat['receita'], 2, ',', '.') ?></span>
                        </div>
                        <div class="cat-bar-bg">
                            <div class="cat-bar-fill" style="width:<?= round($pct) ?>%;background:<?= $cores[$ci % count($cores)] ?>;"></div>
                        </div>
        ...
        <div class="dash-card">
            ...
            <?php endforeach; else: ?>
                <div class="empty-dash">Nenhum pedido no periodo.</div>
            <?php endif; ?>
        </div>

        <!-- VENDAS POR ATENDENTE/CAIXA -->
        ...
        <div class="dash-card" style="margin-top:0.75rem;margin-bottom:0;">
            ...
            <div style="overflow-x:auto;">
                <table class="ranking-table">
                    ...
                    <tbody>
                        <?php 
                        $total_geral_atend = array_sum(array_column($vendas_atendentes, 'total_vendido'));
                        ...
                        <tr>
                            <td>
                                <div style="display:flex;align-items:center;gap:0.5rem;">
                                    <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--purple));display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:0.75rem;flex-shrink:0;">
                                        <?= strtoupper(substr($atend['operador'], 0, 2)) ?>
                                    </div>
                                    <div>
                                        <div style="font-weight:700;font-size:0.85rem;"><?= htmlspecialchars($atend['operador']) ?></div>
                                        <div style="font-size:0.7rem;color:var(--gray);"><?= $pct ?>% do total</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td style="text-align:center;">
                                <button onclick="abrirModalAtendente('<?= htmlspecialchars(addslashes($atend['operador'])) ?>', <?= $pct ?>, '<?= number_format($atend['total_vendido'],2,',','.') ?>', <?= $atend['total_itens'] ?>, <?= $atend['total_comandas'] ?>, '<?= $prods_json ?>')"
                                    style="padding:0.3rem 0.8rem;background:var(--primary);color:#fff;border:none;border-radius:0.4rem;font-size:0.75rem;font-weight:700;cursor:pointer;">
                                    Ver
                                </button>
                            </td>
                        </tr>
        ...
        <div id="modal-atendente" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;padding:1rem;">
            <div style="background:#fff;border-radius:1rem;padding:1.5rem;width:100%;max-width:520px;max-height:85vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.2);animation:slideUp .2s ease;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;">
                    <div style="display:flex;align-items:center;gap:0.75rem;">
                        ...
                        <div>
                            <div id="modal-nome" style="font-size:1rem;font-weight:800;color:var(--dark);"></div>
                            <div id="modal-sub" style="font-size:0.75rem;color:var(--gray);"></div>
                        </div>
                    </div>
                ...
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.6rem;margin-bottom:1.25rem;">
                    <div style="background:#eff6ff;border-radius:0.6rem;padding:0.75rem;text-align:center;">
                        <div style="font-size:1.2rem;font-weight:800;color:var(--primary);" id="modal-total"></div>
                        <div style="font-size:0.65rem;color:var(--gray);font-weight:600;text-transform:uppercase;">Total</div>
                    </div>
                    <div style="background:#f0fdf4;border-radius:0.6rem;padding:0.75rem;text-align:center;">
                        <div style="font-size:1.2rem;font-weight:800;color:var(--success);" id="modal-itens"></div>
                        <div style="font-size:0.65rem;color:var(--gray);font-weight:600;text-transform:uppercase;">Itens</div>
                    </div>
                    <div style="background:#faf5ff;border-radius:0.6rem;padding:0.75rem;text-align:center;">
                        <div style="font-size:1.2rem;font-weight:800;color:var(--purple);" id="modal-cmds"></div>
                        <div style="font-size:0.65rem;color:var(--gray);font-weight:600;text-transform:uppercase;">Comandas</div>
                    </div>
                </div>
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: { position: 'top', labels: { font: { size: isMobile ? 10 : 12 }, boxWidth: 12 } },
                tooltip: { callbacks: { label: ctx => ctx.datasetIndex === 0 ? ' R$ ' + ctx.raw.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : ' ' + ctx.raw + ' pedido(s)' } }
            },
            scales: {
...
function abrirModalAtendente(nome, pct, total, itens, cmds, prodsJson) {
    ...
    const cores = ['#2563eb','#10b981','#8b5cf6','#f59e0b','#ef4444','#06b6d4','#ec4899','#84cc16'];
    ...
    document.getElementById('modal-produtos').innerHTML = prods.length === 0
        ...
        : prods.map((p, i) => {
            ...
            const nomeLimpo = p.nome_produto.replace(/^\[AVULSO\]\s*/, '');
            const cor = cores[i % cores.length];
            return `
            <div style="margin-bottom:0.9rem;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.2rem;">
                    <span style="font-size:0.82rem;font-weight:600;color:#1e293b;max-width:65%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${nomeLimpo}</span>
                    <span style="font-size:0.75rem;font-weight:700;color:${cor};">${pctValor}% · R$ ${parseFloat(p.total).toLocaleString('pt-BR',{minimumFractionDigits:2})}</span>
                </div>
                <div style="height:7px;background:#f1f5f9;border-radius:99px;overflow:hidden;">
                    <div style="width:${pctQtd}%;height:100%;background:${cor};border-radius:99px;transition:width .8s ease;"></div>
                </div>
                <div style="font-size:0.68rem;color:#94a3b8;margin-top:0.15rem;">${p.qtd} unidade(s) vendida(s)</div>
            </div>`;
        }).join('');

    const modal = document.getElementById('modal-atendente');
    modal.style.display = 'flex';
}

function fecharModalAtendente() {
    document.getElementById('modal-atendente').style.display = 'none';
}

document.getElementById('modal-atendente').addEventListener('click', function(e) {
    if (e.target === this) fecharModalAtendente();
});
